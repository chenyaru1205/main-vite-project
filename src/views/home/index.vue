	...
	<div class="home-container">
		<el-row :gutter="15" class="home-card-one">
			...
			>
				<div class="home-card-item flex">
					<div class="flex-margin flex w-[100%]" :class="`home-one-animation${k}`">
						<div class="flex-auto">
							<span class="text-3xl">{{ v.num1 }}</span>
							<span class="ml-[5px] text-base" :style="{ color: v.color1 }">{{ v.num2 }}%</span>
							<div class="mt-[10px]">{{ v.num3 }}</div>
						</div>
					</div>
				</div>
			</el-col>
		...
		<el-row :gutter="15" class="home-card-three">
			<el-col :xs="24" :sm="10" :md="10" :lg="8" :xl="8">
				<div class="home-card-item">
					<div class="home-card-item-title">测试卡片</div>
					<div class="home-monitor">
						<div class="flex-warp">
							<div class="flex-warp-item" v-for="(v, k) in state.homeThree" :key="k">
								<div class="flex-warp-item-box" :class="`home-animation${k}`">
									<div class="flex-margin">
										<span class="p-[l5px]">{{ v.label }}</span>
										<div class="mt-[10px]">{{ v.value }}</div>
									</div>
								</div>
							</div>
						</div>
...
import { reactive, onMounted, ref, nextTick, onActivated, markRaw } from 'vue';
...
const initLineChart = () => {
	...
	const option = {
		...
		legend: { data: ['测试数据', '测试数据bb'], right: 0 },
		xAxis: {
...
const initPieChart = () => {
	...
	const getname = ['设备1', '设备2', '设备3', '设备4', '设备5'];
...
const initBarChart = () => {
	...
	const option = {
		...
		tooltip: { trigger: 'axis' },
		legend: { data: ['测试1', '测试2', '测试3'], right: 0 },
...
.home-container {
    ...
    margin-bottom: 10px;
		.home-card-item {
			...
			&:hover {
				box-shadow: 0 2px 12px var(--color-border-hover);
				transition: all ease 0.3s;
			}
			&-icon {
				width: 70px;
